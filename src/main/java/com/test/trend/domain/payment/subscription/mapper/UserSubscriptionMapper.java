...
public interface UserSubscriptionMapper {
	...
	@Mapping(source = "seqSubscriptionPlan.seqSubscriptionPlan", target = "seqSubscriptionPlan")
    @Mapping(source = "seqSubscriptionPlan.planName", target = "planName")
    UserSubscriptionDTO toDto(UserSubscription entity);

    /** Entity 생성 시 DTO → Entity 직접 매핑할 일 거의 없음 → ignore 처리 */
    @Mapping(target = "seqSubscriptionPlan", ignore = true)
